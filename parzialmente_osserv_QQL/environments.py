...
class ClassicToyEnv:
    ...
    def step(self, action):
        ...
        sp_full = np.random.choice(range(self.nS), size=1, p=self.T[self.currentState, action].squeeze())
        r = self.R[sp_full]

        # Make the state partially observable
        sp_partial = self.get_partial_state(sp_full)

        # Update step count and state
        self.currentState = sp_full
        self.currentStep += 1

        return sp_partial, r

    def get_partial_state(self, full_state):
        # Replace with your specific logic
        partial_state = full_state[:2]  # Take only the first two elements of the state
...
class QuantumToyEnv(ClassicToyEnv):
    ...
    def step(self, action):
        ...
        counter= 0
        a= int(action)
        while a>0 and counter < len(aValue):
            aValue[counter]= (a & 1)
            a>>=1
            counter+= 1
